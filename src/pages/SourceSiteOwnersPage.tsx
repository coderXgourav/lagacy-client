  ...
  return (
    <div className="min-h-[calc(100vh-4rem)] bg-gradient-to-br from-background via-background to-muted/20 p-6">
      <div className="container mx-auto max-w-[1600px] space-y-6">
        ...
        {/* Hero Section */}
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-blue-500/10 via-blue-500/5 to-transparent border border-blue-500/20 p-8 md:p-12 mb-2">
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            <div className="space-y-4 flex-1">
              <div className="flex items-center gap-3">
                ...
                <div className="px-2.5 py-0.5 rounded-full bg-blue-500/20 text-blue-500 border border-blue-500/30 text-xs font-semibold tracking-wide uppercase">
              ...
              <h1 className="text-4xl md:text-5xl font-bold tracking-tight bg-gradient-to-r from-foreground via-foreground/90 to-foreground/70 bg-clip-text text-transparent">
                Source Existing Site Owners
              </h1>
              <p className="text-lg text-muted-foreground max-w-2xl">
                Discover websites built with {selectedTech} and connect with their owners directly. Filter by geography and registration age.
              </p>
            </div>
            <div className="flex flex-col gap-2 pt-2 items-end">
              <Badge variant="outline" className="bg-blue-500/10 text-blue-600 border-blue-500/20 px-3 py-1 mt-2">
                {allDomains.length} domains ready
              </Badge>
            </div>
          </div>
        </div>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                <h2 className="text-2xl font-bold">
                  {selectedTech} Directory
                </h2>
                <Badge variant="secondary" className="px-3 py-1">
                  {filteredDomains.length} results found
                </Badge>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredDomains.map((domain, index) => {
                ...
                return (
                  <Card
                    key={index}
                    className={`overflow-hidden hover:shadow-lg transition-all duration-300 hover:scale-[1.02] cursor-pointer ${isRecent ? "border-green-500/30 bg-green-500/5" : ""
                      }`}
